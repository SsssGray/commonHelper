...
namespace CommonHelper
{
    ...
    public class HttpItem
    {
        ...
        string _UserAgent = "Mozilla/5.0 (Windows NT 6.3; rv:36.0) Gecko/20100101 Firefox/36.04";
        /// <summary>
    ...
    public class HttpAdd
    {
        ...
        /// <returns></returns>
        public static string GetImgAndCooke(string url,ref Image img)
        {
            ...
            HttpResult rst = helper.GetHtml(item);
            byte[] bt = rst.ResultByte;
        ...
        /// <returns></returns>
        public static string OnlyGetHtml(string url)
        {
        ...
        /// <returns></returns>
        public static string OnlyGetHtml(string url, string cookie)
        {
            HttpHelper httphelper = new HttpHelper();
            HttpItem item = new HttpItem()
            {
        ...
        /// <param name="ip"></param>
        /// <returns></returns>
        public static string UseIpGetHtml(string url, string ip)
        {
            HttpHelper httphelper = new HttpHelper();
            HttpItem item = new HttpItem()
            {
        ...
        /// <returns></returns>
        public static string PostGetHtml(string url, string cookie, string postdata)
        {
            HttpHelper httphelper = new HttpHelper();
            HttpItem item = new HttpItem()
            {
        ...
        public static string PostJsonGetHtml(string url, string cookie, string postdata)
        {
            HttpHelper httphelper = new HttpHelper();
            HttpItem item = new HttpItem()
            {
        ...
        public static String PostRetry(string url, string cookie, string postdata,int retryCount,String content)
        {
            String html;
            int errorCount = 0;
            do
            {
                html = PostGetHtml(url, cookie, postdata);
                errorCount++;
            } while (!html.Contains(content) && errorCount < retryCount);
            if (errorCount == retryCount)
            {
                throw new CustomException("无法获取数据");
            }
            return html;
        }
        ...
        /// <param name="refer">来源页</param>
        /// <returns></returns>
        public static string PostGetHtmlXml(string url, string cookie, string postdata)
        {
            HttpHelper httphelper = new HttpHelper();
            HttpItem item = new HttpItem()
            {
        ...
        public static string PostJson(string url, string cookie, string postdata)
        {
            HttpHelper httphelper = new HttpHelper();
            HttpItem item = new HttpItem()
            {
        ...
        /// <returns></returns>
        public static string OnlyGetHtml(string url, string cookie, string refer, string contentType)
        {
            HttpHelper httphelper = new HttpHelper();
            HttpItem item = new HttpItem()
            {
        ...
        /// <returns></returns>
        public static string OnlyGetHtml(string url, Encoding encoding)
        {
            HttpHelper httphelper = new HttpHelper();
            HttpItem item = new HttpItem()
            {
        ...
        public static byte[] GetPicStream(string url)
        {
            HttpItem item = new  HttpItem()
            {
                URL = url,
            ...
            HttpHelper httphelper = new HttpHelper();
            HttpResult httpresult = httphelper.GetHtml(item);
            byte[] bytes = httpresult.ResultByte;
            return bytes;
        }
        #endregion
        ...
        /// </summary>
        /// <param name="url"></param>
        public static ConcurrentStack<string> GetIP()
        {
            //随机获得代理ip的链接
            List<string> ipUrl = new List<string>();
            //ipUrl.Add("http://www.15daili.com/apiProxy.ashx?un=jiejie&pw=jiejie&count=1000");
            //ipUrl.Add("http://www.xsdaili.com/get?orderid=185981378376299&num=2000&area=%E4%B8%AD%E5%9B%BD");
            //ipUrl.Add("http://www.66ip.cn/nmtq.php?getnum=800&isp=0&anonymoustype=0&start=&ports=&export=&ipaddress=&area=1&proxytype=2&api=66ip");
           // ipUrl.Add("http://www.66ip.cn/getzh.php?getzh=2017090465517&getnum=4000&isp=0&anonymoustype=0&start=&ports=&export=&ipaddress=&area=1&proxytype=2&api=https");
            HashSet<string> ipHash = new HashSet<string>();
            Random random = new Random();
            int index = random.Next(0, ipUrl.Count);
            string url = ipUrl[index];
            ...
            while (stack.Count == 0)
            {
                index = random.Next(0, ipUrl.Count);
                url = ipUrl[index];
                html = HttpAdd.OnlyGetHtml(url);
                matches = Regex.Matches(html, @"\d+.\d+.\d+.\d+:\d+");
                foreach (Match match in matches)
                {
                    stack.Push(match.Value);
                }
        ...
        /// 验证代理是否可用，true为可用
        /// </summary>
        /// <param name="ip">需验证的IP</param>
        /// <param name="url">验证网站</param>
        /// <param name="keywords">访问成功关键词</param>
        /// <param name="timeout">访问时间限制</param>
        /// <returns></returns>
        public static bool VerIP(string ip, string url = "https://www.baidu.com/", string keywords = "百度一下", int timeout = 10000)
        {
            HttpHelper httphelper = new HttpHelper();
            HttpItem item = new HttpItem()
            {
        ...
        public static bool pingIP(string ip)
        {
            ...
            PingReply reply = pingSender.Send(ips[0], 1000);//第一个参数为ip地址，第二个参数为ping的时间 
        ...
        /// <param name="cookie"></param>
        /// <returns></returns>
        public static Image GetPicAndCookie(ref string cookie,string url)
        {
            HttpHelper httphelper = new HttpHelper();
            HttpItem item = new HttpItem()
            {
        ...
        /// <returns></returns>
        public static bool AllowUse(string keyWord)
        {

            HttpHelper httphelper = new HttpHelper();
            HttpItem item = new HttpItem()
            {
                URL = "http://47.93.242.107:8001/control.txt",
                UserAgent = "Mozilla/5.0 (Windows NT 10.0; WOW64; rv:54.0) Gecko/20100101 Firefox/54.0",
                ContentType = "application/x-www-form-urlencoded; charset=UTF-8",
                Accept = "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",
                Allowautoredirect = true,
        ...
        public static void NavigateUrl(string url, WebBrowser web)
        {
            if (web.InvokeRequired)
            {
                Action<string> action = a => { web.Navigate(a); web.ScriptErrorsSuppressed = true; };
                web.Invoke(action, url);
            }
            else
            {
                web.Navigate(url);
        ...
        //public  void WINAPI WriteWebBrowserRegKey(LPCTSTR lpKey,DWORD dwValue)
        //{
        //    HKEY hk;
